...
create_user() {
    ...
    if [ $? -eq 0 ]; then
        log_action "User $user created and added to primary group: $user"
    else
        log_action "Creating user $user failed."
        return
    fi
    ...
    log_action "User $user was added to these groups: ${group_array[*]}"
    ...
    log_action "Password for user $user has been set and saved securely."
}
...
echo "User(s) creation completed, kindly check /var/log/user_management.log for more details."
